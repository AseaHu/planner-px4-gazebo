...
void SDFMap::projectDepthImage() {
  ...
  /* use depth filter */
    ...
    if (!md_.has_first_depth_)
      ...
      //std::cout<<"max_ray_length_: "<<mp_.max_ray_length_<<std::endl;
      Eigen::Matrix3d last_camera_r_inv;
      last_camera_r_inv = md_.last_camera_q_.inverse();
      const double inv_factor = 1.0 / mp_.k_depth_scaling_factor_;
      //std::cout<<"k_depth_scaling_factor_: "<<mp_.k_depth_scaling_factor_<<std::endl;
...
void SDFMap::raycastProcess() {
  ...
  //std::cout<<"boundary "<<mp_.map_max_boundary_.transpose()<<std::endl;
...
void SDFMap::depthPoseCallback(const sensor_msgs::ImageConstPtr& img,
  ...
  //cv::imshow("depth", md_.depth_image_);
  //cv::waitKey(0.01);
